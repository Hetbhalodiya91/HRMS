...
import { CalendarPlus, CalendarDays, Clock, CheckCircle, XCircle, Loader } from 'lucide-react'

function StatCard({ icon: Icon, label, value, color, to }) {
  const content = (
    <div className={`card hover:shadow-md transition-shadow cursor-pointer border-l-4 ${color}`}>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-500 mt-1">
          {user?.departmentName ? `${user.departmentName} Department` : 'No department assigned'}
          {' · '}{user?.roles?.join(', ')}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatCard icon={Clock} label="Pending Leaves" value={pending}
          color="border-yellow-400" to="/my-leaves" />
        <StatCard icon={CheckCircle} label="Approved Leaves" value={approved}
          color="border-green-400" to="/my-leaves" />
        <StatCard icon={XCircle} label="Rejected Leaves" value={rejected}
          color="border-red-400" to="/my-leaves" />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Link to="/my-leaves"
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">View all your leave requests</p>
          </div>
        </Link>
      </div>
      ...
      <div className="card">
        ...
        {loading ? (
          ...
          <div className="text-center py-10 text-gray-400">
            ...
            <Link to="/apply-leave" className="text-primary-600 text-sm mt-2 inline-block hover:underline">
